# Clone of AWS VPC Terraform module

This is a modified clone of the AWS VPC module from https://registry.terraform.io/modules/terraform-aws-modules/vpc/aws/latest

The intention is this is only used for our squid use case, and we will get these changes included in the upstream version once tested. All other code should continue to use the version from the terraform registry unless they specifically need the changes included here.

List of changes:
* Removed unnecessary files for local version (circleci files, changelogs etc.)
* Removed unneeded examples, created new example for testing created functionality
* Modified intra subnets to be capable of having a routing table per subnet. This is required to enable proper failover for our proxy network
* Added terratest covering new routing use case
...
| <a name="input_single_intra_route_table"></a> [single\_intra\_route\_table](#input\_single\_intra\_route\_table) | Controls if a single route table is created for intra subnets, or one per subnet | `bool` | `true` | no |
...
| <a name="output_default_vpc_arn"></a> [default\_vpc\_arn](#output\_default\_vpc\_arn) | The ARN of the VPC |
