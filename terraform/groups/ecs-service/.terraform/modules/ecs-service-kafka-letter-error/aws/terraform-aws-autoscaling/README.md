...
| <a name="input_refresh_triggers"></a> [refresh\_triggers](#input\_refresh\_triggers) | (Optional) Set of additional property names that will trigger an Instance Refresh. A refresh will always be triggered by a change in any of launch\_configuration, launch\_template, or mixed\_instances\_policy. | `set(string)` | <pre>[<br>  "launch_configuration"<br>]</pre> | no |
