locals {
  ...
  create_lb_target_group    = var.batch_service || var.multilb_setup ? false : true
  create_lb_listener_rule   = var.batch_service || var.multilb_setup ? false : true
  create_fargate_lb_ingress = var.batch_service || var.multilb_setup ? false : true

  # Shortening the target_group_name in case it's over the 32 char max 
  target_group_name_suffix  = var.use_eric_reverse_proxy ? "-eric" : ""
  full_target_group_name    = var.service_short_name == "" ? "${var.environment}-${var.service_name}${local.target_group_name_suffix}" : "${var.environment}-${var.service_short_name}${local.target_group_name_suffix}"
  ...
  lb_health_check_grace_period_seconds = local.create_lb_target_group ? var.health_check_grace_period_seconds : null
  ...
  # when using eric as a reverse proxy traffic needs to be sent to eric which then forwards to the task container or when
  # target container port supplied use this one
  ...
  eric_healthcheck_definition = var.use_task_container_healthcheck ? local.task_container_eric_healthcheck_definition : ""
  # use_task_container_healthcheck
  task_container_eric_healthcheck_definition = <<DEFINITION
            "healthCheck": {
              "command": [
                  "CMD-SHELL",
                  "[[ $(curl http://localhost:${var.eric_port}/healthcheck -o /dev/null -w '%%{http_code}\n' -s) == '200' ]] || exit 1"
              ],
              "interval": 30,
              "timeout": 5,
              "retries": 3
  ...
  task_container_port_mappings = var.batch_service ? [] : [{
  ...
  task_container_definition = <<DEFINITION
        {
            ...
            "portMappings": ${jsonencode(local.task_container_port_mappings)},
  ...
  eric_container_definition = <<DEFINITION
        {
          ...
          ${local.eric_healthcheck_definition}
  ...
  sidecar_containers = [
    for side_car_container in var.additional_sidecar_containers : {
      name : side_car_container.name
      image : side_car_container.image
      cpu : side_car_container.cpu
      memory : side_car_container.memory
      dependsOn : [
        for dependency in side_car_container.depends_on : {
          containerName : dependency.container_name
          condition : dependency.condition
        }
      ]
      portMappings : [
        for port_mapping in side_car_container.port_mappings : {
      ...
      logConfiguration : {
        ...
        options : {
          "awslogs-create-group" : "true",
          "awslogs-region" : var.aws_region,
          "awslogs-group" : "/ecs/${var.name_prefix}/${side_car_container.name}/${var.service_name}",
          "awslogs-stream-prefix" : "ecs"
        }
      }
      essential : side_car_container.essential
    }
  ]
  ...
  container_definitions                  = concat(local.container_definitions_minus_additional, [for container in local.sidecar_containers : container])
  ...
  target_container               = var.target_container_name == "" ? var.service_name : var.target_container_name
  load_balancer_target_container = var.use_eric_reverse_proxy ? "eric" : local.target_container
}
