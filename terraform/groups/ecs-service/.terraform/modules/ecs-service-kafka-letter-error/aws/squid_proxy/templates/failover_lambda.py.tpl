...
def handler(event, context):
  ...
  for record in event['Records']:
    ...
    if message['NewStateValue'] == 'ALARM':
      ...
      # Route traffic to the first healthy NAT instance
      for healthy_alarm in cw_client.describe_alarms(AlarmNamePrefix='${ALARMPREFIX}/', ActionPrefix='${ALARMTOPIC}', StateValue='OK')['MetricAlarms']:
        healthy_asg_name = healthy_alarm['AlarmName'].split('/')[1]
        healthy_asg = as_client.describe_auto_scaling_groups(AutoScalingGroupNames=[healthy_asg_name])['AutoScalingGroups'][0]
        ...
        for route_table in ec2_client.describe_route_tables(Filters=[{'Name': 'tag:AutoScalingGroupName', 'Values': [asg_name]}])['RouteTables']:
    ...
    # If the NAT instance has recovered
      ...
      lc_name = as_client.describe_lifecycle_hooks(AutoScalingGroupName=asg_name)['LifecycleHooks'][0]['LifecycleHookName']
      try:
      ...
      for route_table_id in as_client.describe_tags(Filters=[
