...
variable "read_only_root_filesystem" {
  type        = bool
  description = "Checks if Amazon Elastic Container Service (Amazon ECS) Containers only have read-only access to its root filesystems. Enabling this setting helps to minimize security vulnerabilities as it prevents unauthorized tampering or writing to the container instance’s filesystem unless explicit read-write permissions are granted to its folders and directories."
  default     = true
}

variable "eric_read_only_root_filesystem" {
  type        = bool
  description = "This is a flag for ERIC container. Checks if Amazon Elastic Container Service (Amazon ECS) Containers only have read-only access to its root filesystems. Enabling this setting helps to minimize security vulnerabilities as it prevents unauthorized tampering or writing to the container instance’s filesystem unless explicit read-write permissions are granted to its folders and directories."
  default     = true
}
...
# ------------------------------------------------------------------------------
variable "batch_service" {
  type        = bool
  default     = false
  description = "Defines whether a service is designed to be batch processing service (true) or not (false); a batch processing service will not have a target group, load balancer listener rules or a fargate security group created for it. Ideal for services that perform periodic processing or simply do not require network access"
}
...
variable "mount_points" {
  ...
  default     = [ {  "sourceVolume": "tmp",  "containerPath": "/tmp",  "readOnly": false  } ]
}

variable "eric_mount_points" {
  type        = list(any)
  description = "Used to define mount points in the eric container definition"
  default     = [ {  "sourceVolume": "tmp",  "containerPath": "/tmp",  "readOnly": false  } ]
}
...
# ------------------------------------------------------------------------------
variable "multilb_setup" {
  type        = bool
  default     = false
  description = "For services that use multiple load balancers. This disables creation of sinle-lb resources, all lb-related resources will need to be created using multilb."
}
...
variable "multilb_listeners" {
  description = "Map of listener details"
  type = map(object({
    listener_arn      = string
...
variable "multilb_max_rules" {
  description = "Maximum number of security groups to be granted access to ECS service fargate sg; here to avoid targeted apply because of dynamic keys unknown to terraform plan."
  type        = number
  default     = 10
}
...
variable "multilb_cloudwatch_alarms_enabled" {
  description = "Whether to create a standard set of cloudwatch alarms for the service in multilb setup.  Requires a notify SNS topic to have already been created for the cluster, or for cloudwatch_alert_notify_topic var to be set so that another topic is used."
  type        = bool
  default     = false
}
...
variable "enable_target_group_stickiness" {
  default     = false
  description = "Determines whether session stickiness is enabled (true) or not (false) on the created target groups"
  type        = bool
}
