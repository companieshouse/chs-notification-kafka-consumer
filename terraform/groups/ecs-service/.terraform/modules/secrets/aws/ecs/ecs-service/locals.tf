locals {
  ...
  # when using eric as a reverse proxy traffic needs to be sent to eric which then forwards to the task container
  traffic_port = var.use_eric_reverse_proxy ? var.eric_port : var.container_port
  ...
  eric_container_definition = <<DEFINITION
        {
          ...
          "readonlyRootFilesystem": ${var.eric_read_only_root_filesystem},
          "mountPoints": ${jsonencode(var.eric_mount_points)},
          "portMappings": [{
  ...
  container_definitions = var.use_eric_reverse_proxy ? "[${local.eric_container_definition}, ${local.task_container_definition}]" : "[${local.task_container_definition}]"
