...
resource "aws_route_table" "public" {
  count = var.create_vpc && length(var.public_subnets) > 0 ? var.single_public_route_table ? 1 : length(var.public_subnets) : 0
...
resource "aws_route" "public_internet_gateway" {
  count = var.create_vpc && var.create_igw && length(var.public_subnets) > 0 ? var.single_public_route_table ? 1 : length(var.public_subnets) : 0
...
resource "aws_route" "public_internet_gateway_ipv6" {
  count = var.create_vpc && var.create_igw && var.enable_ipv6 && length(var.public_subnets) > 0 ? var.single_public_route_table ? 1 : length(var.public_subnets) : 0
...
resource "aws_route_table" "intra" {
  count = var.create_vpc && length(var.intra_subnets) > 0 ? var.single_intra_route_table ? 1 : length(var.intra_subnets) : 0

  vpc_id = local.vpc_id

  tags = merge(
    {
      "Name" = var.single_intra_route_table ? "${var.name}-${var.intra_subnet_suffix}" : format(
  ...
  lifecycle {
    ignore_changes = [
      # Ignore changes to tags to avoid removing tags created by failover Lambda
      tags,
    ]
  }
}
...
resource "aws_vpn_gateway_route_propagation" "public" {
  count = var.create_vpc && var.propagate_public_route_tables_vgw && (var.enable_vpn_gateway || var.vpn_gateway_id != "") ? var.single_public_route_table ? 1 : length(var.public_subnets) : 0
