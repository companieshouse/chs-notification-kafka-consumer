locals {
  waf_v2_policies = length(var.waf_v2_policies) > 0 ? { for policy in flatten(var.waf_v2_policies) : policy.name => policy } : {}
}
...
resource "aws_fms_policy" "waf_v2" {
  ...
  security_service_policy_data {
    ...
    managed_service_data = jsonencode({
      type                                    = "WAFV2"
      customRequestHandling                   = lookup(each.value.policy_data, "customRequestHandling", null)
      customResponse                          = lookup(each.value.policy_data, "customResponse", null)
      sampledRequestsEnabledForDefaultActions = lookup(each.value.policy_data, "sampledRequestsEnabledForDefaultActions", null)
      ...
      loggingConfiguration              = lookup(each.value.policy_data, "logging_configuration", null)
    })
  }
}
